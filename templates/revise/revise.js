...
const $manage_list_menu = document.querySelector("#manage_list_menu");
const localFIleTitle = JSON.parse(localStorage.getItem("fileTitle"));

for (let x of localTitleList) {
  if (x === localFIleTitle) {
    $manage_list_menu.innerHTML +=
...
$manage_list_menu.addEventListener("change", () => {
  const text = $manage_list_menu.options[$manage_list_menu.selectedIndex].value;
...
$default_select.addEventListener("click", (event) => {
  ...
  for (let string of defaultVar) {
    ...
    if (CheckDuplicate(inputValue.join("") + excel_var_text)) {
      // 주간평균(변수명) 중복확인
      if (text.includes("온도")) {
        // 모든 변수 다넣기
        $final_var[0].innerHTML +=
          `<Option value= '${string}${excel_var_text}'>` +
          inputValue.join("") +
          excel_var_text +
          `</option>`;
