...
$environment.addEventListener("change", () => {
  ...
  const environmentTitle =
    $environment.options[$environment.selectedIndex].textContent;
...
$merge.addEventListener("click", async () => {
  ...
  const growthTitle =
    $growthVariable.options[$growthVariable.selectedIndex].value;
  const environmentTitle =
    $environmentVariable.options[$environmentVariable.selectedIndex].value;
  const outputTitle =
    $outputVariable.options[$outputVariable.selectedIndex].value;
...
$save.addEventListener("click", () => {
  console.log("mergeCompleteData",mergeCompleteData.getData());
  //   Loading();
  $.ajax({
    url: "../merge-view/",
    type: "post",
    ...
    data: {
      header: "save",
      fileName: $mergeFileName.value,
      mergedData: JSON.stringify(mergeCompleteData.getData()),
    },
    success: function (response) {
      if (response.data != null) {
