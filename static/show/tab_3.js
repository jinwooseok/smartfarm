// 엑셀 불러오기
// 엑셀 열 선택 및 저장
// 평균 및 최대 최소 등 새로 만들 data에 들어갈 열 js
...
let excel_arr = Object.keys(data[0]); // 전체 엑셀 열 이름
...
window.onload = function () {
    ...
    for (let i = 0; i < Object.keys(data[1]).length; i++) {
        // select box 데이터 추가
        $excel_var.innerHTML += `<Option value= '${Object.keys(data[0])[i]}'>` + Object.keys(data[0])[i] + "</option>";
        // 엑셀 열 제목 추가 및 열 크기 지정
        excel_col[i] = ({ title: Object.keys(data[0])[i], width: '150px' }) // "저장시간":"202206071700" 에서 저장시간 부분
    }
