    ...
    def dateConverter(self):
        ...
        try:
            if date_type == str or date_type == object:
                self.data["날짜"] = pd.to_datetime(self.data["날짜"],format='mixed',yearfirst=True,utc=True,errors='coerce')
            elif date_type in [int, np.int64]:
                self.data["날짜"] = pd.to_datetime(self.data["날짜"].astype(str),format='mixed',yearfirst=True,utc=True,errors='coerce')
