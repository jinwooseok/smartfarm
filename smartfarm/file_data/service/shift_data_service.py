...
class ShiftDataService():
    ...
    def execute(self):
        ...
        if isinstance(self.file_object, File) and FileStatus.objects.filter(file_id=self.file_object.id, status_id=2).exists() == False:
            raise FileNotCompleteException()
        elif isinstance(self.file_object, Temp) and TempStatus.objects.filter(file_id=self.file_object.id, status_id=4).exists() == False:
            raise FileNotCompleteException()
